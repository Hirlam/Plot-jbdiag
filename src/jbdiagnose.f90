...
!
      ...
      real(kind=8)      :: zdummy,zeps,zjn,wgt
...
!
      integer(kind=4)    :: jn_start,jn_stop
...
!
 ...
 1000    format('diag/spdensPP',i2.2)
         open(unit=50,file=fnspdens,form='formatted',status='NEW')
         do jn=1,nsmax_jb_in
            write(50,*)jn,&
...
!
 ...
 6000 format(i3,f10.2,5f16.12,3f8.4)
      close(unit=50)
!
!
!   Wind components level dependence
!
      open(unit=50,file='diag/baloperuv',form='formatted')
      do jj=1,nflevg
...
!
 ...
 6012    format('diag/spdensUV',i2.2)
         open(unit=50,file=fnspdens,form='formatted')
         do jn=1,nsmax_jb_in
            write(50,*)jn,&
              &  gsize_in*float(max(nlon_in,nlat_in))/1000./float(jn),&
